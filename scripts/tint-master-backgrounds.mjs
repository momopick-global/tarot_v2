/**
 * 마스터별 플로우 배경(bg_01~03.png) 색조 적용 — 01_Cassian 제외, 2~9번만
 * tint-master-card-backs.mjs 와 동일한 색·블렌드 기준
 *
 * 실행: node scripts/tint-master-backgrounds.mjs
...
const BG_FILES = ["bg_01.png", "bg_02.png", "bg_03.png"];
...
async function tintPng(inputPath, tint) {
...
async function main() {
  for (const tint of TINTS) {
    for (const name of BG_FILES) {
      const filePath = path.join(root, tint.dir, name);
      try {
        await fs.access(filePath);
      } catch {
        console.warn(`skip (no file): ${tint.dir}/${name}`);
        continue;
      }
      console.log(`${tint.dir}/${name} → rgb(${tint.r},${tint.g},${tint.b})`);
      await tintPng(filePath, tint);
    }
  }
